...
#    def __init__(self, tag_len = 1000, code_len  = 16):
#        nn.Module.__init__(self)
#        self.code_length = code_len
#        self.tag_length = tag_len
#        self.hidden_dim = int(np.ceil((self.tag_length + self.code_length)/2))
#
#        self.feature_extractor = nn.Sequential(
#            nn.Linear(self.tag_length, self.hidden_dim),
#            nn.Linear(self.hidden_dim, self.hidden_dim),
#            nn.Linear(self.hidden_dim, self.code_length)
#        )
#    def forward(self, x):
...
class TextNet(nn.Module):
    def __init__(self, tag_len = 1000, code_len  = 16):
        ...
        #self.hidden_dim = int(np.ceil((self.tag_length + self.code_length)/2))
        self.hidden_dim = int(np.ceil((self.tag_length + self.code_length)/2))
        self.fc1 = nn.Linear(self.tag_length, self.hidden_dim)
        self.fc2 = nn.Linear(self.hidden_dim, self.hidden_dim)
        self.fc3 = nn.Linear(self.hidden_dim, self.code_length)

        self.fc1.weight.data.normal_(0, 0.01)
        self.fc1.bias.data.fill_(0.0)
        self.fc2.weight.data.normal_(0, 0.5)
        self.fc2.bias.data.fill_(0.0)
        self.fc3.weight.data.normal_(0, 0.5)
